...
namespace FF
{
	...
	{
		auto _transform = m_animator->getFinalBoneMatrices();
		for (uint i = 0; i < _transform.size(); i++)
		{
			_shader.setMatrix("_finalBoneMatrices[" + std::to_string(i) + "]", _transform[i]);
		}
	...
	void ffModel::loadBoneWeightForVertices(std::vector<ffVertex>& _vertexArr, 
											aiMesh* _pMesh, 
											const aiScene* _pScene) 
	{
		for (uint _boneindex = 0; _boneindex < _pMesh->mNumBones; _boneindex++) 
		{
			...
			uint			_weightNum = _pMesh->mBones[_boneindex]->mNumWeights;
