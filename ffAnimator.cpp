...
namespace FF 
{
	...
	void ffAnimator::updateAnimation(float _dt, std::map<std::string, ffBoneInfo>& _boneInfoMap)
	{
		m_deltaTime = _dt;
		if (m_currentAnimation)
		{
			...
			this->calculateBoneTransform(&m_currentAnimation->getRootNode(), glm::mat4(1.0f), _boneInfoMap);
		}
	}
	...
	void ffAnimator::calculateBoneTransform(const ffAssimpNodeData* _pNode,
											...
											std::map<std::string, ffBoneInfo >& _boneInfoMap)
	{
		...
		if (_boneInfoMap.find(_nodeName) != _boneInfoMap.end())
		{
			...
			glm::mat4 _offsetMatrx = _boneInfoMap[_nodeName].m_offsetMatrix;
			m_finalBoneMatrices[_boneIndex] = _globalTransformMatrix * _offsetMatrx;
		}
